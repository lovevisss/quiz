<template>
    <div class="space-y-6 p-6">
        ...
        <section class="rounded-lg border bg-white p-4 shadow-sm">
            ...
            <p v-if="form.recentlySuccessful" class="mt-2 text-sm text-emerald-700">标签已添加。</p>
        ...
        <section class="rounded-lg border bg-white p-4 shadow-sm">
            <div class="mb-3 flex items-center justify-between gap-3">
                ...
                <span class="text-sm text-slate-500">共 {{ tags.length }} 个标签</span>
            </div>

            <div v-if="tags.length === 0" class="rounded border border-dashed border-slate-300 bg-slate-50 px-4 py-8 text-center text-sm text-slate-500">
                还没有标签，先添加一个用于题目选择。
            </div>
                ...
                <table class="min-w-full border text-sm">
                    <thead class="bg-slate-50">
                        <tr>
                            ...
                            <th class="border px-3 py-2 text-left">已使用题目数</th>
                    ...
                    <tbody>
                        <tr v-for="tag in tags" :key="tag.id">
                            ...
                            <td class="border px-3 py-2">
                                <span class="inline-flex rounded-full bg-sky-50 px-3 py-1 text-sky-700">
                                    {{ tag.name }}
                                </span>
                            </td>
                            <td class="border px-3 py-2">{{ tag.questions_count }}</td>
                            <td class="border px-3 py-2">
                                <button
                                    type="button"
...
function remove(tag: QuestionTagItem): void {
    ...
    router.delete(`/admin/question-tags/${tag.id}`, {
        preserveScroll: true,
        onError: (errors) => {
            deleteError.value = String(errors.delete ?? '删除失败，请稍后重试。');
        },
        onFinish: () => {
